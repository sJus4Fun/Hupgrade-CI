...
# Brand dart-defines from config/.env (fall back to app_config.dart defaults when not set)
DART_BRAND_DEFINES = --build-dart-define=APP_NAME=$(APP_NAME) --build-dart-define=BUNDLE_ID=$(BUNDLE_ID) --build-dart-define=APP_VERSION=$(APP_VERSION) --build-dart-define=PUBLISHER_URL=$(PUBLISHER_URL) --build-dart-define=PRIVACY_URL=$(PRIVACY_URL) --build-dart-define=TERMS_URL=$(TERMS_URL) --build-dart-define=CONTACT_EMAIL=$(CONTACT_EMAIL) --build-dart-define=CHAT_WIDGET_ID=$(CHAT_WIDGET_ID)

BUILD_ARGS=--dart-define sentry_dsn=$(SENTRY_DSN)
DISTRIBUTOR_ARGS=--skip-clean --build-target $(TARGET) --build-dart-define sentry_dsn=$(SENTRY_DSN) $(DART_BRAND_DEFINES)

# Build-time branding: regenerate packaging configs from .tmpl templates
# 自动在每个 release target 前执行
.PHONY: brand
brand:
	@dart pub global activate fastforge 2>/dev/null || true
	@python -c "from PIL import Image" 2>nul || python3 -c "from PIL import Image" 2>/dev/null || pip3 install Pillow -q || python3 -m pip install Pillow -q --break-system-packages || pip install Pillow -q || python -m pip install Pillow -q --break-system-packages || brew install pillow 2>/dev/null || echo "⚠️  Pillow not found. Run: pip3 install Pillow"
...
# windows-msix-release is commented out — re-enable for MS Store distribution
windows-release: windows-zip-release windows-exe-release # windows-msix-release

windows-zip-release: brand
	dart pub global run fastforge:main package \
	  --platform windows \
	  --targets zip \
	  --skip-clean \
...
macos-release: brand
	dart pub global run fastforge:main package --platform macos --targets dmg,pkg $(DISTRIBUTOR_ARGS)

ios-release: brand #not tested
	dart pub global run fastforge:main package --platform ios --targets ipa --build-export-options-plist  ios/exportOptions.plist $(DISTRIBUTOR_ARGS)
